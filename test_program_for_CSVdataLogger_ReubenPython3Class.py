...
if __name__ == '__main__':
    ...
    SINUSOIDAL_MOTION_INPUT_MaxValue = 20.0
    ...
    while(EXIT_PROGRAM_FLAG == 0 or CSVdataLogger_ReubenPython3ClassObject.IsSaving() == 1):
        ...
        SINUSOIDAL_INPUT_TO_COMMAND_1 = (SINUSOIDAL_MOTION_INPUT_MaxValue + SINUSOIDAL_MOTION_INPUT_MinValue)/2.0 + 0.5*abs(SINUSOIDAL_MOTION_INPUT_MaxValue - SINUSOIDAL_MOTION_INPUT_MinValue)*math.sin(1.0*time_gain*CurrentTime_MainLoopThread)
        SINUSOIDAL_INPUT_TO_COMMAND_2 = (SINUSOIDAL_MOTION_INPUT_MaxValue + SINUSOIDAL_MOTION_INPUT_MinValue)/2.0 + 0.5*abs(SINUSOIDAL_MOTION_INPUT_MaxValue - SINUSOIDAL_MOTION_INPUT_MinValue)*math.cos(1.0*time_gain*CurrentTime_MainLoopThread)

        if CSVdataLogger_OPEN_FLAG == 1:
            CSVdataLogger_ReubenPython3ClassObject.AddDataToCSVfile_ExternalFunctionCall([SINUSOIDAL_INPUT_TO_COMMAND_1, SINUSOIDAL_INPUT_TO_COMMAND_2])
        ####################################################
        ####################################################
            ...
            if "StandAlonePlottingProcess_ReadyForWritingFlag" in MyPlotterPureTkinterStandAloneProcess_ReubenPython2and3ClassObject_MostRecentDict:
                ...
                if MyPlotterPureTkinterStandAloneProcess_ReubenPython2and3ClassObject_MostRecentDict_StandAlonePlottingProcess_ReadyForWritingFlag == 1:
                    if CurrentTime_MainLoopThread - LastTime_MainLoopThread_PLOTTER >= 0.040:
                        MyPlotterPureTkinterStandAloneProcess_ReubenPython2and3ClassObject.ExternalAddPointOrListOfPointsToPlot(["Channel0", "Channel1"], [CurrentTime_MainLoopThread]*2, [SINUSOIDAL_INPUT_TO_COMMAND_1, SINUSOIDAL_INPUT_TO_COMMAND_2])
